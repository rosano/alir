...
(function(global) {
  ...
  var RemoteStorage = function() {
    RemoteStorage.eventHandling(this, "ready", "disconnected", "disconnect", "conflict", "error", "features-loaded", "connecting", "authing", "wire-busy", "wire-done", "sync-interval-change");
    this._pending = [];
    this._setGPD({
  ...
  RemoteStorage.prototype = {
    ...
    _setGPD: function(impl, context) {
      function wrap(f) {
        return function() {
          return f.apply(context, arguments).then(emitUnauthorized.bind(this));
        };
      }
...
(function(global) {
  ...
  RS.WireClient.REQUEST_TIMEOUT = 3e4;
  RS.WireClient.prototype = {
    ...
    get: function(path, options) {
      ...
      var promise = this._request("GET", this.href + cleanPath(path), this.token, headers, undefined, this.supportsRevs, this._revisionCache[path]);
      if (!isFolder(path)) {
        return promise;
      } else {
...
(function(global) {
  ...
  RemoteStorage.Discover = function(userAddress, callback) {
    ...
    var urls = [ "https://" + hostname + "/.well-known/webfinger" + params, "https://" + hostname + "/.well-known/host-meta.json" + params, "http://" + hostname + "/.well-known/webfinger" + params, "http://" + hostname + "/.well-known/host-meta.json" + params ];
    function tryOne() {
      var xhr = new XMLHttpRequest();
      var url = urls.shift();
      if (!url) {
      ...
      xhr.onload = function() {
        ...
        if (link) {
          var authURL = link.properties["http://tools.ietf.org/html/rfc6749#section-4.2"] || link.properties["auth-endpoint"], storageType = link.properties["http://remotestorage.io/spec/version"] || link.type;
...
(function(global) {
  function extractParams() {
    var location = RemoteStorage.Authorize.getLocation(), hashPos = location.href.indexOf("#"), hash;
    if (hashPos === -1) {
      return;
    }
  ...
  RemoteStorage.Authorize._rs_init = function(remoteStorage) {
    ...
    var params = extractParams(), location;
    if (params) {
...
(function(global) {
  ...
  function setModuleCaching(remoteStorage, key) {
    if (key === "*" || key === "") {
      remoteStorage.caching.set("/", {
        data: true
      });
    } else {
      remoteStorage.caching.set("/" + key + "/", {
        data: true
      });
      remoteStorage.caching.set("/public/" + key + "/", {
        data: true
      });
    }
  }
...
(function(pMode) {
  var mode = pMode, env = {}, isBackground = false;
...
(function(window) {
  ...
  RemoteStorage.Widget.View = function(remoteStorage) {
    ...
    RemoteStorage.eventHandling(this, "connect", "disconnect", "sync", "display", "reset");
...
(function(global) {
  ...
  ValidatorContext.prototype.validateAll = function validateAll(data, schema, dataPathParts, schemaPathParts) {
    ...
    var error = this.validateBasic(data, schema) || this.validateNumeric(data, schema) || this.validateString(data, schema) || this.validateArray(data, schema) || this.validateObject(data, schema) || this.validateCombinations(data, schema) || null;
  ...
  ValidatorContext.prototype.validateNumeric = function validateNumeric(data, schema) {
    return this.validateMultipleOf(data, schema) || this.validateMinMax(data, schema) || null;
  };
  ...
  ValidatorContext.prototype.validateString = function validateString(data, schema) {
    return this.validateStringLength(data, schema) || this.validateStringPattern(data, schema) || null;
  };
  ...
  ValidatorContext.prototype.validateArray = function validateArray(data, schema) {
    ...
    return this.validateArrayLength(data, schema) || this.validateArrayUniqueItems(data, schema) || this.validateArrayItems(data, schema) || null;
  };
  ...
  ValidatorContext.prototype.validateObject = function validateObject(data, schema) {
    ...
    return this.validateObjectMinMaxProperties(data, schema) || this.validateObjectRequiredProperties(data, schema) || this.validateObjectProperties(data, schema) || this.validateObjectDependencies(data, schema) || null;
  };
  ...
  ValidatorContext.prototype.validateCombinations = function validateCombinations(data, schema) {
    var error;
    return this.validateAllOf(data, schema) || this.validateAnyOf(data, schema) || this.validateOneOf(data, schema) || this.validateNot(data, schema) || null;
  };
  ...
  function searchForTrustedSchemas(map, schema, url) {
    if (typeof schema.id == "string") {
      if (schema.id.substring(0, url.length) == url) {
        ...
        if (url.length > 0 && url.charAt(url.length - 1) == "/" || remainder.charAt(0) == "#" || remainder.charAt(0) == "?") {
...
(function(global) {
  ...
  RS.BaseClient.prototype = {
    ...
    getAll: function(path) {
      ...
      return this.storage.get(this.makePath(path)).then(function(status, body) {
        ...
        if (typeof body === "object") {
          ...
          for (var key in body) {
            this.storage.get(this.makePath(path + key)).then(function(status, b) {
              body[this.key] = b;
              i++;
              if (i === count) {
    ...
    cache: function(path, enable) {
      ...
      this.storage.caching[enable === false ? "disable" : "enable"](this.makePath(path), this.storage.connected);
      return this;
    },
...
(function(global) {
  ...
  RemoteStorage.Caching.prototype = {
    enable: function(path, waitForRemote) {
      this.set(path, {
        data: true,
        ready: !waitForRemote
      });
    ...
    _query: function(path) {
      return this._pathSettingsMap[path] || path !== "/" && this._query(containingFolder(path));
    },
  ...
  Object.defineProperty(RemoteStorage.Caching.prototype, "list", {
    get: function() {
      ...
      for (var path in this._pathSettingsMap) {
        list.push({
          path: path,
          settings: this._pathSettingsMap[path]
        });
      }
      return list;
    }
...
(function(global) {
  var syncInterval = 1e4, backgroundSyncInterval = 6e4, isBackground = false;
  function isFolder(path) {
  ...
  function handleVisibility() {
    var hidden, visibilityChange, rs = this;
  ...
  function isValidInterval(interval) {
    return typeof interval === "number" && interval > 1e4 && interval < 36e5;
  }
  RemoteStorage.Sync = {
    sync: function(remote, local, path) {
      return pushChanges(remote, local, path).then(function() {
    ...
    return promising(function(promise) {
      ...
      while (path = roots.shift()) {
        (function(path) {
          ...
          RemoteStorage.Sync.sync(rs.remote, rs.local, path, cachingState).then(function() {
...
(function(global) {
  ...
  function keepFolderNode(node) {
    return Object.keys(node.body).length > 0 || Object.keys(node.cached).length > 0;
  }
  ...
  RS.IndexedDB.prototype = {
    ...
    put: function(path, body, contentType, incoming, revision) {
      ...
      transaction.oncomplete = function() {
        ...
        if (!incoming) {
          this._recordChange(path, {
            action: "PUT",
            revision: oldNode ? oldNode.revision : undefined
          });
        }
        if (typeof done === "undefined") {
          done = true;
    ...
    delete: function(path, incoming) {
      var promise = promising();
      if (path[path.length - 1] === "/") {
        throw "Bad: don't DELETE folders";
      }
      var transaction = this.db.transaction([ "nodes" ], "readwrite");
      var nodes = transaction.objectStore("nodes");
      var oldNode;
    ...
    getRevision: function(path) {
      ...
      var transaction = this.db.transaction([ "nodes" ], "readonly");
      var rev;
      transaction.objectStore("nodes").get(path).onsuccess = function(evt) {
    ...
    changesBelow: function(path) {
      ...
      var cursorReq = transaction.objectStore("changes").openCursor(IDBKeyRange.lowerBound(path));
      var pl = path.length;
      var changes = [];
...
(function(global) {
  ...
  RemoteStorage.LocalStorage._rs_cleanup = function() {
    ...
    for (var i = 0; i < l; i++) {
      var key = localStorage.key(i);
      if (key.substr(0, npl) === NODES_PREFIX || key.substr(0, cpl) === CHANGES_PREFIX) {
        remove.push(key);
      }
    }
...
(function(global) {
  ...
  RS.GoogleDrive.prototype = {
    configure: function(_x, _y, _z, token) {
      if (token) {
    ...
    _getFolder: function(path, options) {
      ...
      this._getFileId(path, function(idError, id) {
        ...
        if (idError) {
          ...
          this._request("GET", BASE_URL + "/drive/v2/files?" + "q=" + encodeURIComponent(query) + "&fields=" + encodeURIComponent(fields) + "&maxResults=1000", {}, function(childrenError, response) {
            if (childrenError) {
  ...
  if (document.addEventListener) {
    ...
    if (!window.JSON) {
      getL10nAttributes = function(element) {
        if (!element) return {};
        var l10nId = element.getAttribute("data-l10n-id"), l10nArgs = element.getAttribute("data-l10n-args"), args = {};
        if (l10nArgs) try {
          args = eval(l10nArgs);
        } catch (e) {
          consoleWarn("could not parse arguments for #" + l10nId);
    ...
    if (!document.querySelectorAll) {
      getTranslatableChildren = function(element) {
        if (!element) return [];
        var nodes = element.getElementsByTagName("*"), l10nElements = [], n = nodes.length;
        for (var i = 0; i < n; i++) {
      ...
      getL10nResourceLinks = function() {
        var links = document.getElementsByTagName("link"), l10nLinks = [], n = links.length;
        for (var i = 0; i < n; i++) {
          if (links[i].type == "application/l10n") l10nLinks.push(links[i]);
        }
        return l10nLinks;
      };
    }
...
(function() {
  ...
  var blacklist = {
    tags: [ "script", "style" ],
    attr: [ "onclick", "onerror", "onload", "id", "style", "class" ]
  };
  var htmlParser = function(html, handler) {
      ...
      if (!stack.last() || !special[stack.last()]) {
        if (html.indexOf("<!--") === 0) {
          ...
          if (match) {
            ...
            var altStartTag = /<(\w)+?[^>]*>/;
            match = html.match(altStartTag);
            if (match) {
              html = html.substring(match[0].length);
              match[0].replace(altStartTag, parseStartTag);
              chars = false;
            }
          }
        }
    ...
    function parseEndTag(tag, tagName) {
      var pos;
      if (!tagName) pos = 0; else for (pos = stack.length - 1; pos >= 0; pos--) if (stack[pos] === tagName) break;
      if (pos >= 0) {
      ...
      start: function(tag, attrs, unary) {
        ...
        if (blacklist.tags.indexOf(tag) === -1) {
          ...
          for (var i = 0; i < attrs.length; i++) {
            ...
            if (blacklist.attr.indexOf(attrs[i].name) === -1) {
...
Showdown.converter = function(a) {
  ...
  this.makeHtml = function(a) {
    ...
    a = M(a), a = a.replace(/^[ \t]+$/gm, ""), Showdown.forEach(f, function(b) {
      a = k(b, a);
    }), a = z(a), a = m(a), a = l(a), a = o(a), a = K(a), a = a.replace(/~D/g, "$$"), 
  ...
  var k = function(a, b) {
    ...
    a = a.replace(/^[ ]{0,2}([ ]?\_[ ]?){3,}[ \t]*$/gm, b), a = x(a), a = y(a), a = E(a), 
    a = m(a), a = F(a), a;
  }, p = function(a) {
    return a = B(a), a = q(a), a = H(a), a = t(a), a = r(a), a = I(a), a = G(a), a = D(a), 
    a = a.replace(/  +\n/g, " <br />\n"), a;
  }, q = function(a) {
    var b = /(<[a-z\/!$]("[^"]*"|'[^']*'|[^'">])*>|<!(--.*?--\s*)+>)/gi;
  ...
  var y = function(a) {
    ...
    return a = a.replace(/(^|[^\\])(`+)([^\r]*?[^`])\2(?!`)/gm, function(a, b, c, d, e) {
      var f = d;
      return f = f.replace(/^([ \t]*)/g, ""), f = f.replace(/[ \t]*$/g, ""), f = C(f), 
      b + "<code>" + f + "</code>";
    }), a;
  }, C = function(a) {
...
var Gesture = function() {
  "use strict";
  var cursor, allowedEvent = [ "gestureStart", "gestureMove", "gestureEnd", "gesture" ], listeners = [], startTarget, endTarget;
  cursor = {
    startX: 0,
    startY: 0
  };
  ...
  function onTouchStart(event) {
    ...
    listeners.forEach(function(elmt) {
      if (elmt.contains(ev.target)) {
        elmt.dispatchEvent(new CustomEvent("gestureStart", {
          detail: ev
        }));
      }
    });
  ...
  function onTouchMove(event) {
    ...
    listeners.forEach(function(elmt) {
      if (elmt.contains(ev.target)) {
        elmt.dispatchEvent(new CustomEvent("gestureMove", {
          detail: ev
        }));
      }
    });
  ...
  function onTouchEnd(event) {
    var ev = getEvent(event), delta = 10, dirs = [ "N", "NW", "W", "SW", "S", "SE", "E", "NE", "N" ], dir = false;
    endTarget = ev.target;
    ...
    listeners.forEach(function(elmt) {
      if (elmt.contains(ev.target)) {
        elmt.dispatchEvent(new window.CustomEvent("gestureEnd", {
          detail: ev
        }));
        if (dir) {
          elmt.dispatchEvent(new window.CustomEvent("gesture", {
            detail: {
              dir: dir,
  ...
  function attach(element, events) {
    var names = Object.keys(events), remaining = names.filter(function(e) {
      return allowedEvent.indexOf(e) === -1;
    });
...
var matchesSelector = document.documentElement.matches || document.documentElement.matchesSelector || document.documentElement.webkitMatchesSelector || document.documentElement.mozMatchesSelector || document.documentElement.oMatchesSelector || document.documentElement.msMatchesSelector;
...
var utils = {
  ...
  log: function log() {
    "use strict";
    var args = Array.prototype.slice.call(arguments), level, levelNum, message, ui, curDate = utils.getDate().substr(11, 8);
    if (args.length > 1) {
      level = args.pop();
    } else {
      level = "info";
  ...
  createXPathFromElement: function(elm) {
    "use strict";
    var allNodes = document.getElementsByTagName("*"), uniqueIdCount, i, n, sib, segs;
...
var Tiles = function(global) {
  ...
  var current, tiles = [], popup = window.matchMedia("(min-width: 78rem) and (min-height: 30rem)").matches;
  ...
  this.go = function(name, cb) {
    var i = tiles.findIndex(function(e) {
      return e.name === name;
    }), next, popupElmt;
    if (i !== -1) {
      next = tiles[i];
...
View.toDom = function(str, fullUrl) {
  "use strict";
  var fragment = document.createDocumentFragment(), sandbox = document.createElement("div"), domain = new RegExp("((http.?://[^/]+).*/)([^?]*).?(.*$)").exec(fullUrl), domainUrl, baseUrl;
  try {
    if (domain !== null) {
      domainUrl = domain[2];
      baseUrl = domain[1];
    ...
    Array.prototype.forEach.call(sandbox.querySelectorAll("* img:not([src^=http])"), function(e) {
      var src = e.getAttribute("src"), cachedImg;
      if (src) {
        if (src.substr(0, 1) === "/") {
          src = domainUrl + src;
        } else {
...
View.template = function(sel, data) {
  "use strict";
  var re = new RegExp("{{([=#].*?)}}", "g"), frag, xpathResult, i, elmt, attr;
        ...
        if (tmp.length > 0) {
          while ((fct = tmp.shift()) !== undefined) {
            switch (utils.trim(fct).toLowerCase()) {
             ...
             case "tolocal":
              res = new Date(res).toLocaleString(undefined, {
                day: "2-digit",
                month: "2-digit",
                year: "numeric",
                hour: "2-digit",
...
function Article() {
  "use strict";
  var self = this, currentId, tags = [], dynamicSheet = document.getElementById("dynamicCss").sheet;
  function doNote(event) {
    var res = utils.parent(event.target, function(el) {
      return typeof el.dataset.key !== "undefined";
    });
    if (res !== null) {
  ...
  function prepare(obj) {
    var title = obj.title || obj.id, data;
    ...
    data = {
      ...
      notes: Object.keys(obj.notes).map(function(e, i) {
        return {
          id: e,
          url: obj.id + "/" + e,
          content: obj.notes[e].content
        };
      }),
      flags: typeof obj.flags === "object" ? Object.keys(obj.flags).filter(function(e) {
        return obj.flags[e] === true;
      }).join(",") : ""
    };
      ...
      obj.tags.forEach(function(tag) {
        if (tags.indexOf(tag) === -1) {
          ...
          (function(tag) {
            ...
            View.insertInList(document.getElementById("tagList"), "li", elmt, function(e) {
              return e.dataset.tag.toLowerCase() > tag.toLowerCase();
            });
          })(tag);
        }
      });
  ...
  this.addToList = function(obj) {
    ...
    View.insertInList(document.getElementById("list"), "[data-key]", item, function(e) {
      return e.dataset.date < obj.date;
    });
    if (classes.length !== 0) {
  ...
  this.display = function(obj, update) {
    ...
    if (typeof obj.notes === "object") {
      ...
      Object.keys(obj.notes).forEach(function(noteId, i) {
        var note = obj.notes[noteId], target, container, a;
  ...
  this.create = function() {
    ...
    choices.push({
      ...
      l10nId: "articleCreateArticle"
    });
    choices.push({
      object: "articles",
      method: "createUrl",
      l10nId: "articleCreateUrl"
    });
  ...
  this.save = function() {
    var tile = document.querySelector("[data-tile=articleEdit]"), id = $('#articleEditTile [name="id"]').value, article;
  ...
  this.show = function show(key) {
    ...
    function doShow() {
      var clItem = $('[data-key="' + key + '"]').classList, clMenu = $("#menu").classList, clList = $("#main").classList;
      clMenu.add("detail");
      clMenu.remove("list");
      clList.add("detail");
      clList.remove("list");
  ...
  this.onShown = function onShown() {
    var iframe = document.getElementById("articleShowTile").querySelector("iframe"), scroll;
    if (iframe) {
  ...
  this.hide = function hide() {
    var clMenu = document.getElementById("menu").classList, clMain = document.getElementById("main").classList, current = $('#list > [data-key="' + currentId + '"]');
    if (current) {
  ...
  this.switchTag = function(key, tag) {
    var node = $('#list > [data-key="' + key + '"]'), tags = node.dataset.tags.split(",").filter(function(e) {
      return e !== "";
    }), i = tags.indexOf(tag);
    if (i !== -1) {
      tags.splice(i, 1);
    ...
    self.read(key, function(article) {
      if (article) {
        article.tags = tags.filter(function(t) {
          return t !== "";
        });
        remoteStorage.alir.saveArticle(article);
  ...
  this.share = function(key) {
    if (typeof window.MozActivity !== "undefined") {
      var node = $('#list > [data-key="' + key + '"]'), request = new window.MozActivity({
        name: "share",
        data: {
          type: "url",
  ...
  this.ui = {
    menu: function(folded) {
      var cl = $("#articleShowTile .articleMenu").classList, actions = $("#articleShowTile .articleMenu .articleActions");
      function style() {
    ...
    deleteArticles: function() {
      ...
      if (toDel.length > 0) {
        if (window.confirm(_("articlesDelete", {
          nb: toDel.length
        }))) {
          toDel.forEach(function(elmt) {
            var parent = utils.parent(elmt, function(e) {
              return typeof e.dataset.key !== "undefined";
            }), key;
            if (parent) {
...
window.Comment = function() {
  ...
  this.save = function() {
    var noteId = $('#noteEditTile [name="noteId"]').value, articleId = $('#noteEditTile [name="articleId"]').value;
    if (!noteId) {
      noteId = utils.uuid();
    }
    window.articles.read(articleId, function(article) {
  ...
  this.delete = function() {
    var noteId = $('#noteViewTile [name="noteId"]').value, articleId = $('#noteViewTile [name="articleId"]').value;
...
function Feeds() {
  "use strict";
  var _cache = {}, self = this;
  this.cache = function(obj) {
    _cache[obj.url] = obj;
  };
  this.display = function(obj) {
    var title = obj.title || obj.id, data = {}, item, classes = [];
    ...
    View.insertInList(document.getElementById("feedsList"), "[data-key]", item, function(e) {
      return e.dataset.title.toLowerCase() > title.toLowerCase();
    });
  ...
  this.save = function() {
    var $ = tiles.$("feedEdit"), id = $('[name="id"]').value, obj;
  ...
  this.fetch = function(url, test, cb) {
    ...
    function onFeedGot(err, doc) {
      var feedUpdated, root, format = "atom", cache, parser;
      cache = _cache[url];
      ...
      if (err) {
        ...
        [].slice.call(doc.querySelectorAll(root)).forEach(function(entry) {
          ...
          itemContent = getVal("encoded") || getVal("content") || getVal("summary") || getVal("description");
  ...
  this.test = function(url) {
    ...
    window.feeds.fetch(url, true, function(items) {
      items = items || {};
      window.alir.ui.message(_("feedTestResult", {
        nb: Object.keys(items).length
      }), "info");
      items = {};
    });
  };
  ...
  this.edit = function(url) {
    var $ = tiles.$("feedEdit"), feed = _cache[url];
  ...
  this.show = function(url) {
    var feed = _cache[url], parent = $("#feedDetail");
  ...
  this.delete = function(url) {
    var feed = _cache[url];
    if (window.confirm(_("feedConfirmDelete", {
      title: feed.title
    }))) {
...
if (navigator.mozAlarms) {
  window.alarms = {
    ...
    plan: function() {
      ...
      request.onsuccess = function() {
        var alarms = this.result, current = new Date(), nb = alarms.length, interval = window.config.alarmInterval || 60;
  ...
  window.alarms = {
    ...
    plan: function() {
      ...
      var current = new Date(), nb = this.all.length, interval = window.config.alarmInterval || 60;
...
(function(global) {
  global.saxParser = function saxParser(elem, callbacks) {
    ...
    function parse(node) {
      var name = node.tagName.toLowerCase(), attributeNodes = node.attributes;
      ...
      for (var i = 0; i < num; i++) {
        ...
        if (nodeType === 3) callbacks.ontext(childs[i].textContent); else if (nodeType === 1) parse(childs[i]);
      }
...
(function(global) {
  ...
  Element.prototype = {
    addInfo: function() {
      var info = this.info, childs = this.children, childNum = childs.length, elem;
      for (var i = 0; i < childNum; i++) {
        ...
        if (typeof elem === "string") {
          ...
          for (var j in elem.info.tagCount) {
            if (j in info.tagCount) info.tagCount[j] += elem.info.tagCount[j]; else info.tagCount[j] = elem.info.tagCount[j];
          }
          if (elem.name in info.tagCount) info.tagCount[elem.name] += 1; else info.tagCount[elem.name] = 1;
        }
      }
    ...
    getOuterHTML: function() {
      ...
      if (this.children.length === 0) {
        if (this.name in formatTags) return ret + "/>"; else return ret + "></" + this.name + ">";
      }
    ...
    getInnerHTML: function() {
      ...
      for (var i = 0, j = nodes.length; i < j; i++) {
        if (typeof nodes[i] === "string") ret += nodes[i]; else ret += nodes[i].getOuterHTML();
      }
      return ret;
    },
    getFormattedText: function() {
      var nodes = this.children, ret = "";
      for (var i = 0, j = nodes.length; i < j; i++) {
        if (typeof nodes[i] === "string") ret += nodes[i].replace(re_whitespace, " "); else {
    ...
    getTopCandidate: function() {
      var childs = this.children, topScore = -Infinity, score = 0, topCandidate, elem;
      for (var i = 0, j = childs.length; i < j; i++) {
        ...
        if (childs[i].isCandidate) {
          ...
          score = Math.floor((elem.tagScore + elem.attributeScore) * (1 - elem.info.density));
          if (topScore < score) {
  ...
  Readability.prototype._convertLinks = function(path) {
    ...
    if (path_split[0] === "") path_split.shift(); else Array.prototype.unshift.apply(path_split, this._url.path);
  ...
  Readability.prototype._getBaseURL = function() {
    ...
    var cleaned = "", elementNum = this._url.path.length - 1;
    for (var i = 0; i < elementNum; i++) {
      cleaned += "/" + this._url.path[i].replace(re_extension, "");
    }
    var first = this._url.full.replace(re_params, "").replace(/.*\//, ""), second = this._url.path[elementNum];
  ...
  Readability.prototype._scanLink = function(elem) {
    ...
    var score = 0, linkData = text + elem.elementData;
    if (re_nextLink.test(linkData)) score += 50;
    if (re_pages.test(linkData)) score += 25;
    if (re_final.test(linkData)) {
      if (!re_nextLink.test(text)) if (!(this._scannedLinks[href] && re_nextLink.test(this._scannedLinks[href].text))) score -= 65;
    }
  ...
  Readability.prototype.onattribute = function(name, value) {
    ...
    if (name === "href" || name === "src") {
      if (re_protocol.test(value)) elem.attributes[name] = value; else elem.attributes[name] = this._convertLinks(value);
    } else if (name === "id" || name === "class") {
      value = value.toLowerCase();
      if (!this._settings.weightClasses) ; else if (re_safe.test(value)) {
        elem.attributeScore += 300;
        elem.isCandidate = true;
      } else if (re_negative.test(value)) elem.attributeScore -= 25; else if (re_positive.test(value)) elem.attributeScore += 25;
      elem.elementData += " " + value;
    } else if (elem.name === "img" && (name === "width" || name === "height")) {
      value = parseInt(value, 10);
      if (value !== value) ; else if (value <= 32) {
        elem.name = "script";
  ...
  Readability.prototype.onclosetag = function(tagName) {
    ...
    if (this._settings.stripUnlikelyCandidates && re_unlikelyCandidates.test(elem.elementData) && !re_okMaybeItsACandidate.test(elem.elementData)) {
      return;
    }
    if (tagName === "div" && elem.children.length === 1 && typeof elem.children[0] === "object" && elem.children[0].name in unpackDivs) {
    ...
    if (tagName in embeds) {
      ...
      var p = elem.info.tagCount.p || 0, contentLength = elem.info.textLength + elem.info.linkLength;
      if (contentLength === 0) {
    ...
    filterEmpty: if ((tagName in removeIfEmpty || !this._settings.cleanConditionally && tagName in cleanConditionally) && elem.info.linkLength + elem.info.textLength === 0 && elem.children.length !== 0) {
      for (i = 0, j = okayIfEmpty.length; i < j; i++) {
        if (okayIfEmpty[i] in elem.info.tagCount) break filterEmpty;
      }
      return;
    }
    if (this._settings.replaceImgs && tagName === "a" && elem.children.length === 1 && elem.children[0].name === "img" && re_imgUrl.test(elem.attributes.href)) {
      elem = elem.children[0];
      elem.attributes.src = elem.parent.attributes.href;
    }
    elem.parent.children.push(elem);
    if (tagName === "p" || tagName === "pre" || tagName === "td") ; else if (tagName === "div") {
  ...
  var getCandidateSiblings = function(candidate) {
    var ret = [], childs = candidate.parent.children, childNum = childs.length, siblingScoreThreshold = Math.max(10, candidate.totalScore * .2);
    for (var i = 0; i < childNum; i++) {
      if (typeof childs[i] === "string") continue;
      if (childs[i] === candidate) ; else if (candidate.elementData === childs[i].elementData) {
        ...
        if (childs[i].info.textLength >= 80 && childs[i].info.density < .25) ; else if (childs[i].info.textLength < 80 && childs[i].info.density === 0 && re_sentence.test(childs[i].toString())) ; else continue;
      } else continue;
      ret.push(childs[i]);
    }
    return ret;
  ...
  Readability.prototype.getTitle = function() {
    ...
    if (/ [\|\-] /.test(curTitle)) {
      ...
      if (curTitle.split(" ", 3).length !== 3) curTitle = this._origTitle.replace(/.*?[\|\-] /, "");
    } else if (curTitle.indexOf(": ") !== -1) {
      curTitle = curTitle.substr(curTitle.lastIndexOf(": ") + 2);
      if (curTitle.split(" ", 3).length !== 3) curTitle = this._origTitle.substr(this._origTitle.indexOf(": "));
    }
  ...
  Readability.prototype.getHTML = function(node) {
    ...
    return node.getInnerHTML().replace(/(?:<br\/>(?:\s|&nbsp;?)*)+(?=<\/?p)/g, "").replace(/(?:\s|&nbsp;?)+(?=<br\/>)/g, "").replace(/(?:<br\/>){2,}/g, "</p><p>").trim();
  };
  ...
  Readability.prototype.getArticle = function(type) {
    ...
    if (type === "text") ret.text = this.getText(elem); else ret.html = this.getHTML(elem);
    return ret;
  };
...
function scrap(url, cb) {
  ...
  function onComplete(err, doc) {
    if (err) {
      ...
      try {
        var parser, readable = new window.Readability(), root, parsed;
...
window.Network = function() {
  "use strict";
  var self = this, timeout = 2e4, cache = [], retryInterval = 6e4;
  this.maxRetry = 5;
  function store(reason, url, cb) {
    var found, item;
    found = cache.find(function(e) {
      return e.url === url;
    });
  ...
  this.fetch = function(url, cb) {
    ...
    try {
      ...
      xhr.onload = function(e) {
        clearTimeout(timer);
        var i = cache.findIndex(function(e) {
          return e.url === url;
        });
        if (i !== -1) {
          cache.splice(i, 1);
